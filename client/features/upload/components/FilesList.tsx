...
import { Card, CardBody, CardHeader, Divider, Button, Tabs, Tab, Spinner } from "@heroui/react";
...
export const FilesList = () => {
    ...
    const handleSelectoEnd = (e: any) => {
        ...
        setSelectedFiles(prev => {
            const newSelection = files.filter(file => selectedIds.includes(file.id));
            
            if (e.inputEvent?.shiftKey || e.inputEvent?.ctrlKey || e.inputEvent?.metaKey) {
                // Добавляем к существующему выделению
                const existingIds = prev.map(f => f.id);
                const uniqueNewFiles = newSelection.filter(file => !existingIds.includes(file.id));
                ...
                {selectedFiles.length > 0 && (
                    <div className="flex gap-2 w-full">
                        ...
                        <Button
                            ...
                            className="flex-1"
                        >
                            Скачать {selectedFiles.length > 1 ? `(${selectedFiles.length})` : ''}
                        </Button>
                        )
                        }
                        {selectedType === EFileType.TRASH && (
                            ...
                            >
                                {isRestoring ? 'Восстановление...' : `Восстановить (${selectedFiles.length})`}
                            </Button>
                        )}
                        <Button
                            size="sm"
                        ...
                        >
                            {isDeleting ? 'Удаление...' : selectedType === EFileType.TRASH 
